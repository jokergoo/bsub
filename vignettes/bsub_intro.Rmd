...
**For DFKZ ODCF users: The _bsub_ package works on the submission nodes, the
computing nodes and the Rstudio node.If the current node is not the submission
node, a connection via ssh is automatically established. You might be asked to
enter password for the connection.**
...
will be removed after the job is finished, but note if you have multiple jobs, each job
will save the image in a different file.
