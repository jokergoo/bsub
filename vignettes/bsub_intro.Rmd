...
- `job_log(job_id)` prints the log of a specified running/finished/failed job.
...
- `monitor()` opens a shiny-app-based job monitor.
...
3. Bash environment. It is basically for finding `bsub`/`bjobs`/... Following is how we set up
on our cluster.
